...
-- ChecklistItem -> Checklist -> Trip -< owners (pivot) and users (pivot)
-- Models the query:
--   ChecklistItem.where(formId = ?)
--     .whereHas(checklist, sq =>
--       sq.whereHas(trip, sq2 =>
--         sq2.whereHas(owners)(_.id === userId) || sq2.whereHas(users)(_.id === userId)))
--     .exists()
...
-- Trips
insert into cl_trip values (1, 'Paris');     -- owners: Alice; users: Bob
insert into cl_trip values (2, 'Tokyo');     -- owners: Bob; users: Charlie
insert into cl_trip values (3, 'London');    -- owners: Charlie; users: (none)

-- Trip owners
insert into cl_trip_owner values (1, 1);  -- Paris owned by Alice
insert into cl_trip_owner values (2, 2);  -- Tokyo owned by Bob
insert into cl_trip_owner values (3, 3);  -- London owned by Charlie

-- Trip users
insert into cl_trip_user values (1, 2);   -- Paris used by Bob
insert into cl_trip_user values (2, 3);   -- Tokyo used by Charlie

-- Checklists
insert into cl_checklist values (1, 1, 'Paris packing');    -- trip: Paris
insert into cl_checklist values (2, 2, 'Tokyo packing');    -- trip: Tokyo
insert into cl_checklist values (3, 3, 'London packing');   -- trip: London

-- Checklist items (form_id groups them)
insert into cl_checklist_item values (1, 1, 100, 'Pack passport');     -- checklist 1 (Paris), form 100
insert into cl_checklist_item values (2, 1, 100, 'Pack sunscreen');    -- checklist 1 (Paris), form 100
insert into cl_checklist_item values (3, 2, 100, 'Pack umbrella');     -- checklist 2 (Tokyo), form 100
insert into cl_checklist_item values (4, 3, 200, 'Pack jacket');       -- checklist 3 (London), form 200
insert into cl_checklist_item values (5, 3, 100, 'Pack tea');          -- checklist 3 (London), form 100
