...
/** Compile-time evidence that entity E has a field annotated with @updatedAt. */
trait HasUpdatedAt[E]:
  def column: Col[?]
  def index: Int

object HasUpdatedAt:
  inline given derived[E](using Mirror.ProductOf[E]): HasUpdatedAt[E] =
    ${ derivedImpl[E] }

  private def derivedImpl[E: Type](using Quotes): Expr[HasUpdatedAt[E]] =
    import quotes.reflect.*

    val annotSym = TypeRepr.of[updatedAt].typeSymbol
    ...
    if annotatedIndices.isEmpty then
      report.errorAndAbort(
        s"${TypeRepr.of[E].show} has no field annotated with @updatedAt. " +
          "Add @updatedAt to the timestamp field to derive HasUpdatedAt."
      )
    if annotatedIndices.size > 1 then
      val fieldNames = annotatedIndices.map(_._1.name).mkString(", ")
      report.errorAndAbort(
        s"${TypeRepr.of[E].show} has multiple fields annotated with @updatedAt ($fieldNames). " +
          "Only one @updatedAt field is allowed."
      )
    ...
    if !isInstant && !isOffsetDateTime then
      report.errorAndAbort(
        s"@updatedAt field '$fieldName' must be Instant or OffsetDateTime, " +
    ...
    val tableExpr: Expr[Table] =
      DerivingUtil.tableAnnot[E] match
        ...
        case None =>
          report.errorAndAbort(
            s"${TypeRepr.of[E].show} must have @Table annotation to derive HasUpdatedAt"
          )
