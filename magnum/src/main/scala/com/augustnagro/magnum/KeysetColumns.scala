...
class KeysetColumns[E, C <: Selectable, K <: Tuple] private[magnum] (
  ...
  def asc[A](f: C => ColRef[A], nullOrder: NullOrder = NullOrder.Default)(using codec: DbCodec[A]): KeysetColumns[E, C, Tuple.Append[K, A]] =
    val colRef = f(cols)
  ...
  def desc[A](f: C => ColRef[A], nullOrder: NullOrder = NullOrder.Default)(using codec: DbCodec[A]): KeysetColumns[E, C, Tuple.Append[K, A]] =
    val colRef = f(cols)
    val colIndex = meta.columns.indexWhere(_.scalaName == colRef.scalaName)
