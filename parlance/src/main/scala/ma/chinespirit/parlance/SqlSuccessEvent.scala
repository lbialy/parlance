...
class SqlSuccessEvent private[parlance] (
    /** The SQL String */
    val sql: String,
    anyParams: Any,
    /** Time taken to execute the query, fetch data, and build the results. Does not include time to construct the preparedStatement. For
      * streaming methods like `Query.iterator`, sqlExecTime is only calculated for the first fetch.
      */
