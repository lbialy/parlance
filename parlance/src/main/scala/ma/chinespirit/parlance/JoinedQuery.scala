...
class JoinedQuery[R <: NonEmptyTuple] private[parlance] (
    ...
    private[parlance] val predicate: Option[Predicate],
  ...
  private[parlance] def buildFromJoin: String =
    s"FROM ${metas(0).tableName} t0" + buildJoinsSql

  private[parlance] def joinOnParams: Seq[Any] =
    joinClauses.flatMap(_.onCondition.params)

  private[parlance] def joinOnWriter: FragWriter =
...
object JoinedQuery:
  ...
  private[parlance] def ofImpl[R: Type, T: Type](using Quotes): Expr[Any] =
  ...
  private[parlance] def ofLeftImpl[R: Type, T: Type](using Quotes): Expr[Any] =
    import quotes.reflect.*
