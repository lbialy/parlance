...
extension [A](col: ColRef[A])
  ...
  def ===(value: A)(using codec: DbCodec[A]): WhereFrag =
    ...
    WhereFrag(Frag(s"${col.queryRepr} = ${codec.queryRepr}", Seq(value), writer))
  ...
  infix def in(values: Iterable[A])(using codec: DbCodec[A]): WhereFrag =
    val vals = values.toVector
    if vals.isEmpty then WhereFrag(Frag("1 = 0", Seq.empty, FragWriter.empty))
    else
      val placeholders = vals.map(_ => codec.queryRepr).mkString(", ")
      val writer: FragWriter = (ps, pos) =>
        var currentPos = pos
        for v <- vals do
  ...
  def between(low: A, high: A)(using codec: DbCodec[A]): WhereFrag =
    ...
    WhereFrag(Frag(s"${col.queryRepr} BETWEEN ${codec.queryRepr} AND ${codec.queryRepr}", Seq(low, high), writer))
