...
private val Log = System.getLogger("ma.chinespirit.parlance")
...
private[parlance] def timed[T](f: => T): (T, FiniteDuration) =
...
private[parlance] def batchUpdateResult(updateCounts: Array[Int]): BatchUpdateResult =
  boundary:
...
private[parlance] def elemNames[Mels: Type](res: List[String] = Nil)(using
    Quotes
): List[String] =
  import quotes.reflect.*
  Type.of[Mels] match
...
private[parlance] def elemTypes[Mets: Type](res: List[Type[?]] = Nil)(using
    Quotes
): List[Type[?]] =
  import quotes.reflect.*
  Type.of[Mets] match
...
private[parlance] def sqlNameAnnot[T: Type](elemName: String)(using
    Quotes
): Option[Expr[SqlName]] =
  import quotes.reflect.*
...
private[parlance] def handleQuery[A](sql: String, params: Any)(
