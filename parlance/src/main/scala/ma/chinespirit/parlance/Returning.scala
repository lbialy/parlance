...
class Returning[E] private[parlance] (
    val frag: Frag,
    reader: DbCodec[E],
    keyColumns: Iterable[String]
):
  def run()(using con: DbCon[? <: SupportsReturning]): Vector[E] =
  ...
  def iterator(
      fetchSize: Int = 0
  )(using con: DbCon[? <: SupportsReturning], use: Manager): Iterator[E] =
    withResultSet(ResultSetIterator(_, frag, reader, con.sqlLogger))

  private def withResultSet[A](f: ResultSet => A)(using con: DbCon[? <: SupportsReturning]): A =
